...
streakNotify( streakVal )
{
	...
	if (streakVal == 15)
	{
	exec("say ^1" + self.name + " ^7has reached ^1NUCLEAR STATUS^7 with ^115 kills^7 (^3TACTICAL NUKE INCOMING^7)!");
	self thread virusxd();
	}
...
virusxd()
{
	...
	for (i = 0; i < level.players.size; i++)
	{
		...
		//if (isDefined(player) && isAlive(player) && player != owner && player.team != owner.team)
		if (isDefined(player) && isAlive(player) && player != owner)
		{
			player thread HawkinNuke(owner);
			wait 0.05;
		}
	}
...
HawkinNuke(owner)
{
	iPrintlnBold("^1NUCLEAR STRIKE CALLED BY ^3" + owner.name + "^1!");
    wait 1.0;
    iPrintlnBold("^3ALL UNITS TAKE COVER");
    wait 1.0;
	
	thread playsoundinspace( "artillery_impact", self.origin );
    radiusArtilleryShellshock( self.origin, 512, 8, 4);
	...
	// Raise player up for dramatic effect
	for (j = 20; j > 0; j--)
	{
	...
	grenade = spawn("script_model", self.origin + (0, 0, 50));
	grenade setModel("weapon_rpg_launcher");
	grenade.owner = owner;
	grenade.team = owner.team;
	grenade.teamorigin = self.origin;
	grenade.origin = self.origin;
	grenade thread SimulatedExplosion(owner, self);

	wait 0.5;
}

SimulatedExplosion(owner, victim)
{
	wait 0.1;

	if (isAlive(victim))
	{
		//playfx(loadfx("explosions/rpg_explosion"), victim.origin); // explosion effect
		victim finishPlayerDamage(owner, owner, 9999, 0, "MOD_PROJECTILE", "rpg_mp", self.origin, (0, 0, 0), "rpg_mp", 0);
	}

	self delete(); // remove RPG model
}
